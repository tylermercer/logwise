            ...
            const [logs, forms, entries] = await db.transaction('rw', db.tables, async (trans) => {
                ...
                const groups = await formsToMigrate.reduce(
                    async (groupedPromise: Promise<Map<FormId,OldFormRaw[]>>, form: OldFormRaw) => {
                        const latestVersionId = await findLatestVersionId(form);
...
await db.forms.where('schemaVer').equals(1).modify({ schemaVer: undefined, logId: undefined });
await db.logs.where('schemaVer').equals(1).delete();
await db.entries.where('schemaVer').equals(1).modify({ schemaVer: undefined });
